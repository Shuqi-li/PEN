...
from DataPipe_d import DataPipe
from ConfigLoader_d import logger
...
class Executor:
    ...
    def generation(self, sess, phase):
        ...
        for gen_batch_dict in generation_gen:
            ...
            gen_batch_y, gen_batch_y_, gen_batch_loss, gen_batch_P = sess.run([self.model.y_T, self.model.y_T_, self.model.loss, self.model.P],
                                                                 feed_dict=feed_dict)
            ...
            gen_P_list.append(gen_batch_dict['stock_batch'])
            gen_P_list.append(gen_batch_dict['day_tar'])
            gen_P_list.append(gen_batch_dict['Ts'])
            gen_P_list.append(gen_batch_dict['T_batch'])
            gen_P_list.append(gen_batch_dict['n_msgs_batch'])
            gen_P_list.append(gen_batch_P)
            ...
            for epoch in range(self.model.n_epochs):
                ...
                for train_batch_dict in train_batch_gen:
                    ...
                    if n_iter >= self.silence_step and n_iter % self.skip_step == 0:
                        ...
                        #res, _ = self.generation(sess, phase='dev')
    ...
    def restore_and_test(self):
        with tf.Session(config=self.tf_config) as sess:
            ...
            max_acc = 0.0
            for i in range(10):
                res, P= self.generation(sess, phase='test')
                if res['acc'] > max_acc:
                    max_acc = res['acc']
                    tmp = res 
                    final_P = P 
            stat_logger.print_eval_res(tmp, use_mcc=True)
            '''
            file = open('p2.txt', 'w')
            for i in final_P:
